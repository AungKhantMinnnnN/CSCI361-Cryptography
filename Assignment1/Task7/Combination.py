...
def syncCipherEncrypt(message, key):
    ...
    for i, char in enumerate(message):
        if char.isalpha():
            ...
            encryptedValue = (charValue + keystream[i]) % 26
...
def syncCipherDecrypt(ciphertext, key):
    ...
    for i, char in enumerate(ciphertext):
        if char.isalpha():
            # Convert to 0-25
            charValue = ord(char.upper()) - ord('A')
            decryptedValue = (charValue - keystream[i]) % 26
...
def combinedEncryption(message, cfbKey, syncKey, iv):
    ...
    oddEncryption = CFB1BitEncryptionAndDecryption(oddChars, cfbKey, iv)

    # Encrypt even chars with synchronous cipher
    evenEncryption = syncCipherEncrypt(evenChars, syncKey)
...
def combinedDecryption(cipherText, cfbKey, syncKey, iv):
    ...
    oddDecryption = CFB1BitEncryptionAndDecryption(oddChars, cfbKey, iv)

    # Decrypt even chars with synchronous cipher
    evenDecryption = syncCipherDecrypt(cipherText, syncKey)
